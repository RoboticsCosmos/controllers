...
void biasAdapter_sched(double const * e_bar, double const * bias_threshold, double const * bias_step, double * adapted_bias) {
  ...
  double absolute_decision = 0.0L;
  double difference = 0.0L;
  double absolute_difference = 0.0L;

  /* fixed data flow schedule */
  /* decision map gainDecision */
  difference = (*e_bar) - (*bias_threshold);
  if      (difference > 0.0) decision_access = 1.0;
  else if (difference < 0.0) decision_access = -1.0;
  else decision_access = 0.0;

  absolute_difference = fabs(*e_bar) - (*bias_threshold);
  if      (absolute_difference > 0.0) absolute_decision =  1.0;
  else if (absolute_difference < 0.0) absolute_decision = -1.0;
  else absolute_decision = 0.0;

  bias_step_access = (*bias_step) * (0.5 * (absolute_decision + 1)) * (decision_access);
...
void gainAdapater_sched(double const * e_bar, double const * gain_threshold, double const * gain_step, double * adapted_gain) {
  ...
  /* fixed data flow schedule */
  /* decision map gainDecision */
  difference = fabs(*e_bar) - (*gain_threshold);
  if      (difference > 0.0) decision_access =  1.0;
  else if (difference < 0.0) decision_access = -1.0;
  else decision_access = 0.0;
  ...
  *adapted_gain = +(*adapted_gain) + (gain_step_access);
  ...
  if      ((*adapted_gain) > 1.) *adapted_gain = 1.0;
  else if ((*adapted_gain) < 0.) *adapted_gain = 0.0;
...
void abag_sched(abagState_t * abagState, double const * error, double * actuation, double const * alpha, double const * biasThreshold,
                double const * biasStep, double const * gainThreshold, double const * gainStep) {
  /* data block declarations */
